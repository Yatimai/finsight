...
def mock_pipeline(config):
    with (
        ...
        # Default: retriever returns pages via search_single
        fake_pages = [FakePage(page_number=5), FakePage(page_number=12)]
        p.retriever.search_single.return_value = fake_pages
...
class TestPipelineFlow:
    async def test_full_pipeline_flow(self, mock_pipeline):
        ...
        mock_pipeline.retriever.search_single.assert_called_once()
    ...
    async def test_no_pages_returns_abstention(self, mock_pipeline):
        ...
        mock_pipeline.retriever.search_single.return_value = []
...
class TestPipelineErrorHandling:
    async def test_service_unavailable_returns_error_message(self, mock_pipeline):
        mock_pipeline.generator.generate.side_effect = ServiceUnavailableError("API down")
    ...
    async def test_unexpected_error_returns_generic_message(self, mock_pipeline):
        mock_pipeline.generator.generate.side_effect = RuntimeError("boom")
