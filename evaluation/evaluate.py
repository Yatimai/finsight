...
def load_ground_truth(path: Path) -> list[GroundTruthItem]:
    ...
    for item in items:
        is_abstention = item.category.value == "abstention"
        has_document = bool(item.source_document.strip())
        has_pages = bool(item.source_pages)
        if is_abstention:
            if has_document or has_pages:
                raise ValueError(f"{item.id}: abstention questions must not define source_document or source_pages")
        else:
            if not has_document:
                raise ValueError(f"{item.id}: non-abstention questions must define source_document")
            if not has_pages:
                raise ValueError(f"{item.id}: non-abstention questions must define source_pages")
    return items
