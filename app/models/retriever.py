"""
Retriever: ColQwen2.5 text encoding → Qdrant two-stage MaxSim search.
...
class Retriever:
    ...
    def __init__(self, config: AppConfig, encoder=None, qdrant_client: QdrantClient | None = None):
        ...
        if qdrant_client is not None:
            ...
            self.client = QdrantClient(path=config.qdrant.path, timeout=config.qdrant.timeout_seconds)
        else:
            self.client = QdrantClient(url=config.qdrant.remote_url, timeout=config.qdrant.timeout_seconds)
    ...
    def retrieve(
        ...
        """
        Full retrieval pipeline: encode query → two-stage MaxSim search → return top-k.

        Args:
            queries: Single-element list with the user query
        ...
        query_embeddings = [self.encode_query(q) for q in queries]
        pages = self.search_single(query_embeddings[0], top_k=top_k)

        return pages[:top_k], query_embeddings
